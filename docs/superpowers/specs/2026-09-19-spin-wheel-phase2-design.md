...
| `Frame 1618875132.png` | `assets/images/wheel/wheel-border-ring.png` **(already copied — padded, see below)** | Static ornate border around the wedges (in Hub) |
...
- `src/components/wheel/WheelSegments.tsx` **(modified)**:
  - **Wedge radius no longer fills the full canvas.** Phase 1a's wedges used `r = size / 2` — their own outer arc was the wheel's only visible "rim". Now that the real `wheel-border-ring.png` overlay exists (in `Hub`), the wedges must stop at that ring's *inner* edge instead, or they'd render underneath/behind the border artwork at the wrong radius.

    The source export (`Frame 1618875132.png`, 1080×1131px) was **not square, and its inner hole was not centered** in it either — flood-filled the alpha channel to find the hole's exact connected-component bounding box (not a single row/col guess): 889×889px, centered at `(537, 566)` against a canvas center of `(540, 565.5)` — a real, if small, `(-3, +0.5)`px offset. Fixed at import time rather than compensated for in code: padded to a **1131×1131 square** (29px transparent padding on the left, 22px on the right, 0 top/bottom) so the hole lands at `(566, 566)`, 0.5px from the new canvas center of `(565.5, 565.5)` — the remaining half-pixel is just integer-grid rounding, not a real offset. Saved as `assets/images/wheel/wheel-border-ring.png` (this is the asset actually committed to the repo now, ahead of the rest of the Build order's step 3 — precise enough work that redoing it later would be wasted effort).

    With the asset now a true square, `Hub` renders it at plain `size × size` (no `contentFit` letterboxing concern — a square image in a square box needs none). `WHEEL_INNER_RADIUS_RATIO = (888 / 1131) * 1.01 ≈ 0.793` (888 = the hole's diameter by simple max−min, vs. the 889 inclusive-pixel-count above — same measurement, one-off counting convention, immaterial at this scale). The `×1.01` is a deliberate small overshoot so the wedges' outer edge tucks slightly under the ring's inner edge rather than stopping exactly at it, avoiding a visible anti-aliasing seam. Wedge radius becomes `r = (size / 2) * WHEEL_INNER_RADIUS_RATIO`, with `cx`/`cy` unchanged (still `size / 2` — only `r` shrinks, wedges stay centered).
...
- `src/components/wheel/Hub.tsx` **(modified)** — replaces the Phase 1a placeholder shapes with the real static assets (via `expo-image`), all non-rotating: `wheel-border-ring.png` rendered at plain `size × size` (the asset is now a true 1131×1131 square, centered-hole — see Asset inventory and the wedge-radius math above for how it was fixed), `wheel-hub-flower.png` + `wheel-hub-swan.png` centered (replaces the plain circle), and `wheel-pointer.svg` — a small pennant, native aspect `37:28` — positioned **visually estimated (not measured — no Figma file access, no saved reference screenshot to pixel-check against)**: centered horizontally (`x_center = size/2`), width `≈ 0.14 * size`, height `= width * (28/37) ≈ 0.106 * size`, vertical center at `y_center ≈ 0.38 * size` (i.e. sitting over the *upper* portion of the hub medallion, well short of the outer rim — matching the "on top of the hub, not at the rim" correction, not Phase 1a's rim-edge triangle placement). Flag this for on-device visual tuning once `Hub` actually renders flower + swan + pennant together — proportions eyeballed from memory of the mockup, not derived from source coordinates.
